...
def test_can_authenticate(client):
    client.check_auth()
    assert client._http_client._token is not None
    assert len(client._http_client._token) > 0
...
def test_get_balance_returns_data(client, first_account):
    balance = client.accounts.get_balance(first_account["accountId"])
...
def test_get_transactions_returns_list(client, first_account):
    transactions = client.accounts.get_transactions(first_account["accountId"])
...
def test_get_pending_transactions(client, first_account):
    pending = client.accounts.get_pending_transactions(first_account["accountId"])
...
def test_get_beneficiaries_returns_list(client):
    beneficiaries = client.payments.get_beneficiaries()
    assert isinstance(beneficiaries, list)


@requires_credentials
def test_get_beneficiary_categories_returns_list(client):
    categories = client.payments.get_beneficiary_categories()
...
def test_get_documents_returns_list(client, first_account):
    documents = client.documents.get_documents(
        first_account["accountId"],
        from_date="2026-01-01",
        to_date="2026-04-30",
    )
