...
from krabs_services.finance.investec_client import InvestecPaymentsClient
...
class TransferFundsTool:
    name = "transfer_funds"
    description = "Transfer funds from an Investec account. Only call this after the user has explicitly confirmed the exact transfer details."
    input_schema = TransferFundsInput

    def __init__(self, payments_client: InvestecPaymentsClient) -> None:
        self._payments_client = payments_client

    async def run(self, input_data: TransferFundsInput) -> list[dict[str, Any]]:
        return self._payments_client.transfer_funds(
...
class GetBeneficiariesTool:
    ...
    description = "Get a list of Investec beneficiaries available to the authenticated user."
    ...
    async def run(self, input_data: GetBeneficiariesInput) -> list[dict[str, Any]]:
        _ = input_data
        return self._payments_client.get_beneficiaries()


class GetBeneficiaryCategoriesTool:
    name = "get_beneficiary_categories"
    description = "Get Investec beneficiary categories available to the authenticated user."
    ...
    async def run(self, input_data: GetBeneficiaryCategoriesInput) -> list[dict[str, Any]]:
        _ = input_data
        return self._payments_client.get_beneficiary_categories()


class PayBeneficiariesTool:
    name = "pay_beneficiaries"
    description = "Pay one or more Investec beneficiaries from an account. Only call this after the user has explicitly confirmed the exact payment details."
    ...
    async def run(self, input_data: PayBeneficiariesInput) -> list[dict[str, Any]]:
        return self._payments_client.pay_beneficiaries(
            account_id=input_data.account_id,
